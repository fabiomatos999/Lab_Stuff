...
public class P3Wrapper {
	...
	public static class BinarySearchTree<K, V> implements Map<K, V> {
        ...
        private ArrayList<KeyValuePair<K,V>> ret = new ArrayList<KeyValuePair<K,V>>();
		...
		public ArrayList<KeyValuePair<K, V>> inRangeValues(K key1, K key2) {
            ...
            if(root != null)
            traverseTree(root, key1, key2);
            L = ret;
			return L;			
		}

        private void traverseTree(BinaryTreeNode<MapEntry<K,V>> currNode, K key1, K key2)
        {
            if (currNode == null)
                return;
            if(checkKeys(currNode,key1,key2))
                ret.add(currNode.getValue());
            traverseTree(currNode.getLeftChild(), key1, key2);
            traverseTree(currNode.getRightChild(), key1, key2);
        }

        private boolean checkKeys(BinaryTreeNode<MapEntry<K,V>> currNode, K key1, K key2)
        {
                if(currNode != null && (Integer)currNode.getValue().getKey() > (Integer)key1 && (Integer)currNode.getValue().getKey() < (Integer)key2)
                {
                    return true;
                }
                return false;
